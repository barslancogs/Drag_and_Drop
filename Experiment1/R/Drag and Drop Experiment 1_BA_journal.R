...
seq_5act_strategy.s <- seq_5act_strategy
seq_5act_strategy.s$Condition = factor(seq_5act_strategy.s$Condition, levels = c("1", "2", "3", "4","5"))
contrasts(seq_5act_strategy.s$Condition) = contr.sum(5)
contrasts(seq_5act_strategy.s$Condition)

m1 <- MCMCglmm(new_strategy ~ 1+ Condition+ trait*Condition,
...
seq_5act_wo_outliers_strategy_score1<-subset(seq_5act_wo_outliers_strategy, ItemFinalScoreBinary ==1)
...
################################################################
# Time by strategy and condition and item
################################################################

group.mean_strategy_cond_time_item <- summarySE(seq_5act_wo_outliers_strategy, measurevar = "itemtime", groupvars = c( "Condition", "new_strategy", "item"), na.rm = TRUE)
group.mean_strategy_cond_time_item[group.mean_strategy_cond_time_item$N==1,c("sd", "se", "ci")] <- 0.0 # in case if there is only one subject in a condition, sd, se, ci will be 0.0


ggplot(data=group.mean_strategy_cond_time_item, aes(x=item, y=log(itemtime), fill=new_strategy)) + 
    geom_bar(position=position_dodge(), stat="identity") + 
    geom_errorbar(aes(ymin=log(itemtime)-se, ymax=log(itemtime)+se), width=.2, position=position_dodge(.9)) +
    labs(title="Average time spent in each condition ", 
         x="Condition", y="Average time spent (sec)", size=geom.text.size) +  
    theme_bw() + 
    theme(plot.title=element_text(size=theme.size, face="bold"), axis.title=element_text(size=theme.size, face="bold"), 
          axis.text.x=element_text(size=theme.size), axis.text.y=element_text(size=theme.size),
          legend.text=element_text(size=theme.size), legend.title=element_text(size=theme.size, face="bold")) + 
    scale_fill_discrete(name = "Strategy")+facet_grid(~Condition)


fileName <- "avg_timespentbycondbystrategyitem_bar"
savefig(fileName, 600, 12, 8, "in", "png")

###only for people who scored =1
seq_5act_strategy_score1_item<-subset(seq_5act_only5_wo_outliers_strategy, ItemFinalScoreBinary ==1)
group.mean_strategy_cond_time_score1_item <- summarySE(seq_5act_strategy_score1_item, measurevar = "itemtime", groupvars = c( "Condition", "new_strategy","item"), na.rm = TRUE)

group.mean_strategy_cond_time_score1_item[group.mean_strategy_cond_time_score1_item$N==1,c("sd", "se", "ci")] <- 0.0 # in case if there is only one subject in a condition, sd, se, ci will be 0.0


ggplot(data=group.mean_strategy_cond_time_score1_item, aes(x=item, y=itemtime, fill=new_strategy)) + 
    geom_bar(position=position_dodge(), stat="identity") + 
    geom_errorbar(aes(ymin=itemtime-se, ymax=itemtime+se), width=.2, position=position_dodge(.9)) +
    labs(title="Average time spent in each condition- Score 1 only ", 
         x="Condition", y="Average time spent (sec)", size=geom.text.size) +  
...
fileName <- "avg_timespentbycondbystrategy_Score1_item_bar"
savefig(fileName, 600, 12, 8, "in", "png")


###item 7 only
seq_5act_strategy_score1_item7<-subset(seq_5act_wo_outliers_strategy, ItemFinalScoreBinary ==1 & item == 7 & Condition !=4)
group.mean_strategy_time_score1_item7 <- summarySE(seq_5act_strategy_score1_item7, measurevar = "itemtime", groupvars = c( "new_strategy","item"), na.rm = TRUE)

group.mean_strategy_time_score1_item7[group.mean_strategy_time_score1_item7$N==1,c("sd", "se", "ci")] <- 0.0 # in case if there is only one subject in a condition, sd, se, ci will be 0.0
...
###Condition 1 and Condition 3 only-- for model
seq_5act_strategy_condition1_3<-subset(seq_5act_wo_outliers_strategy, Condition ==1 | Condition == 3)
seq_5act_strategy_condition1_3<-subset(seq_5act_wo_outliers_strategy, item == 4)
group.mean_strategy_time_condition1_3 <- summarySE(seq_5act_strategy_condition1_3, measurevar = "itemtime", groupvars = c( "new_strategy"), na.rm = TRUE)

group.mean_strategy_time_condition1_3[seq_5act_strategy_condition1_3$N==1,c("sd", "se", "ci")] <- 0.0 # in case if there is only one subject in a condition, sd, se, ci will be 0.0


ggplot(data=group.mean_strategy_time_condition1_3, aes(x=new_strategy, y=itemtime)) + 
    geom_bar(position=position_dodge(), stat="identity") + 
    geom_errorbar(aes(ymin=itemtime-se, ymax=itemtime+se), width=.2, position=position_dodge(.9)) +
    labs(title="Average time spent in Condition 1 and 3 ", 
         x="Condition", y="Average time spent (sec)", size=geom.text.size) +  
    theme_bw() + 
    theme(plot.title=element_text(size=theme.size, face="bold"), axis.title=element_text(size=theme.size, face="bold"), 
          axis.text.x=element_text(size=theme.size), axis.text.y=element_text(size=theme.size),
