...
##first exclude Source and Target Focus strategy from strategy column
...
m1 <- MCMCglmm(new_strategy ~  trait+ Condition,
...
# remove outliers
seq_5act_time_wo_outliers <- seq_5act_fredtime_[seq_5act_fredtime_$itemtime > quantile(seq_5act_fredtime_$itemtime, .25) - 1.5*IQR(seq_5act_fredtime_$itemtime) & 
                                                    seq_5act_fredtime_$itemtime< quantile(seq_5act_fredtime_$itemtime, .75) + 1.5*IQR(seq_5act_fredtime_$itemtime), ] #rows

boxplot(seq_5act_time_wo_outliers$itemtime, horizontal = T)


# raw time values

group.mean_new_<- summarySE(seq_5act_time_wo_outliers, measurevar = "itemtime", groupvars = c("new_strategy"), na.rm = TRUE)
...
seq_5act_time_wo_outliers$new_strategy<- relevel(seq_5act_time_wo_outliers$new_strategy, ref = "Source Focus")

strategy_time<-lmer(itemtime ~ new_strategy +(1|ID) +(1|item), data =seq_5act_time_wo_outliers)
